...
class Blackboard {
 public:
  ...
  explicit Blackboard(const std::string &proto_file_path):
    ...
    info.start_pose = InitMapPose();
    ROS_INFO("initial last position");
    info.last_position.pose.position.x = 1;
    info.last_position.pose.position.y = 1;
    // initalize threshold----------------------------------------------------------------------
 ...
 void BuffCallback(const roborts_msgs::BonusStatus::ConstPtr &msg){
    if (info.team_blue) { 
         info.has_buff = bool(msg->blue_bonus ==2);
         info.buff_refresh = bool(msg->blue_bonus ==0);
    }
    else{ 
        info.has_buff = bool(msg->red_bonus == 2);
        info.buff_refresh = bool(msg->red_bonus ==0);
    }
  ...
  void GameStatusCallback(const roborts_msgs::GameStatus::ConstPtr &msg){
      ...
      ROS_INFO("referee---------------------game_status_:%i !",info.game_status);
  }
/**
  void GameStatusCallback(const roborts_msgs::GameStatus::ConstPtr & game_info)
      ...
      remaining_time_ = static_cast<unsigned int>(game_info->remaining_time);
          ROS_INFO("referee---------------------game_status_:%i !",game_status_);
      }
**/
...
//受到攻击时得到转向角
      int  ArmorDamageSource(){
        ROS_INFO("blackboard=====ArmorDamageSource");
        ...
        ROS_INFO("blackboard=====ArmorDamageSource===damage_source_:%i",damage_source_);
        ROS_INFO("blackboard=====ArmorDamageSource===median:%i",median);
        if(turn_time < 0.5 ){
      ...
      {
      ROS_INFO("=====black===RobotDamageCallback");
      damage_type_ = static_cast<int>(robot_damage->damage_type);
      damage_source_ = static_cast<int>(robot_damage->damage_source); 
      info.damage_source_info = static_cast<int>(robot_damage->damage_source); 
      ROS_INFO("=====black===damage_source_:%i",info.damage_source_info);
      }
     //获取机器人正方向到敌方坐标的角度
      double GetEnemyYaw(){
      ...
      double HurtedPerSecond() {
        ...
        if (time_diff > 0.5) {  
          ...
          return self_dmp_;
            ...
            ROS_INFO("meet_else_condition_remain_hp: %d",info.remain_hp);
     ...
     //每秒敌人掉血量
      double HurtPerSecond() {
        ...
        if (time_diff > 0.5) {
          ...
          ROS_INFO("meet_if_condition_enemy_remain_hp: %d",info.remain_hp);
          ...
          return enemy_dmp_;
            ...
            ROS_INFO("meet_else_condition_enemy_remain_hp: %d",info.remain_hp);
	...
	//相对于队友，自己是否距离buff区更近
      bool NearBuff(){
          geometry_msgs::PoseStamped myPose = GetRobotMapPose();
         double dis_my =  GetDistance(info.my_reload, myPose);
         geometry_msgs::PoseStamped allyPose = info.ally;
         double dis_ally = GetDistance(info.ally, myPose); 
        if(dis_my<=dis_ally){
          return true;
        }else{
          return false;
      ...
      //判断是否马上要刷新buff区状态
      bool IsNearRefresh(){
        if((info.remaining_time - 60 >=0) && (info.remaining_time - 60 <=1.5)){
          return true;
        }
        if((info.remaining_time - 120 >=0) && (info.remaining_time - 120 <=1.5)){
          return true;
        } else{
          return false;
          }
...
// add end
 ...
 private:
  ...
  void _ShootThread(int hz){
    // 注意这个函数的具体实现 是 shoot_bahavior.h 中的Run函數
      // ros::Rate loop(hz);
      // while (ros::ok()){
      //     roborts_msgs::ShooterCmd cmd;
      //     cmd.is_shoot = true;
  ...
  void UpdateRobotPose() {
   ...
   // printf("robot_tf_pose:%f",robot_tf_pose.x);
   // ROS_INFO("robot_tf_pose.x:%f",robot_tf_pose.pose.position.x);
  //  ROS_INFO("robot_tf_pose.y:%f",robot_tf_pose.pose.position.y);
    robot_tf_pose.frame_id_ = pose_frameID; // "base_link"
    robot_tf_pose.stamp_ = ros::Time();
    try {
      geometry_msgs::PoseStamped robot_pose;
      tf::poseStampedTFToMsg(robot_tf_pose, robot_pose);
      //ROS_INFO("robot_tf_pose:%f",robot_tf_pose.position.x);
      //ROS_INFO("robot_tf_pose:%f",robot_tf_pose.position.y);
      //ROS_INFO("robot_pose:%f\n",robot_pose.pose.position.x);
      //ROS_INFO("robot_pose:%f\n",robot_pose.pose.position.y);   
      tf_ptr_->transformPose("map", robot_pose, self_pose_);
     // ROS_INFO("robot_pose:%f\n",robot_pose.pose.position.x);
     // ROS_INFO("robot_pose:%f\n",robot_pose.pose.position.y);  
     // ROS_INFO("self_pose_:%f\n",self_pose_.pose.position.x);
     // ROS_INFO("self_pose_:%f\n",self_pose_.pose.position.y);  
    }
