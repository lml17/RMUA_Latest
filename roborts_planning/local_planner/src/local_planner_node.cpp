...
void LocalPlannerNode::ExcuteCB(const roborts_msgs::LocalPlannerGoal::ConstPtr &command) {
  ...
  if (plan_mtx_.try_lock()) {
    ...
    ROS_INFO("local_goal.x:%f",local_goal_.pose.position.x);
    ROS_INFO("local_goal.y:%f",local_goal_.pose.position.y);
...
void LocalPlannerNode::Loop() {
  ROS_INFO("======local_planner_node=================Loop()");
  ...
  ROS_INFO("======local_planner_node=================publish");
//  for (int i = 0; i < 10; ++i) {
    vel_pub_.publish(cmd_vel_);
//    usleep(5000);
//  }
...
int main(int argc, char **argv) {
  ROS_INFO("======local_planner_node=================main");
