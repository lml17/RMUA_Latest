...
GlobalPlannerNode::GlobalPlannerNode() :
 ...
 ROS_INFO("Global planner initial-----------------------------.");
  if (Init().IsOK()) {
...
void GlobalPlannerNode::GoalCallback(const roborts_msgs::GlobalPlannerGoal::ConstPtr &msg) {
  ...
  SetGoal(msg->goal);
   ROS_INFO("GoalCallback x:%f",msg->goal.pose.position.x);
   ROS_INFO("GoalCallback y:%f",msg->goal.pose.position.y);
...
void GlobalPlannerNode::PlanThread() {
  ...
  geometry_msgs::PoseStamped current_goal;
   ROS_INFO("Plan Thread current_start.x:%f",current_start.pose.position.x);
  ROS_INFO("Plan Thread current_start.y:%f",current_start.pose.position.y);
  ROS_INFO("Plan Thread current_goal.x:%f",current_goal.pose.position.x);
  ROS_INFO("Plan Thread current_goal.y:%f",current_goal.pose.position.y);
...
int main(int argc, char **argv) {
  printf("global_planner_node.cpp_____main");
  ROS_INFO("global_planner_node.cpp_____main");
