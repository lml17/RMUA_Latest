...
bool LocalizationNode::Init() {
  ...
  private_nh.param<double>("initial_pose_x", initial_pose_x, -0.189);
  private_nh.param<double>("initial_pose_y", initial_pose_y, -0.124);
  ...
  ROS_INFO("initial_pose_x:%f",initial_pose_x);
  ROS_INFO("initial_pose_y:%f",initial_pose_y);
  ROS_INFO("initial_pose_a:%f",initial_pose_a);
...
//  printf("=====if end \n");
 ...
 // printf("=======TransformLaserscanToBaseFrame( \n");
 ...
 // printf("========LOG_ERROR_IF( \n");
  ROS_INFO("==localnode==hyp_pose_==x:%f",hyp_pose_.pose_mean[0]);
  ROS_INFO("==localnode==hyp_pose_==y:%f",hyp_pose_.pose_mean[1]);
  if(publish_visualize_){
  //  printf("if(publish_visualize_) \n");
     ROS_INFO("==localnode==publish_visualize_");
    PublishVisualize();
  }else{
     ROS_INFO("N==localnode==publish_visualize_");
  }
 // printf("=======IF( end \n");
}
...
void LocalizationNode::PublishVisualize(){
  ...
  if(pose_pub_.getNumSubscribers() > 0){
    ...
    ROS_INFO("==localnode==publish==x:%f",pose_msg_.pose.position.x);
    ROS_INFO("==localnode==publish==y:%f",pose_msg_.pose.position.y);
    pose_pub_.publish(pose_msg_);
  }

  if(particlecloud_pub_.getNumSubscribers() > 0){
    ROS_INFO("============particlecloud");
  ...
  if(!publish_first_distance_map_) {
    ROS_INFO("============!publish_first_distance_map_");
