...
void Amcl::UpdateOdomPoseData(Vec3d pose) {
  ROS_INFO("Amcl::UpdateOdomPoseData");
  Vec3d delta;
  delta.setZero();
...
//    update = true;
  ...
  if (!pf_init_) {
   ...
   // ROS_INFO("odom_data.x:%f",odom_data.pose[0]);
  //  ROS_INFO("odom_data.y:%f",odom_data.pose[1]);
  }
...
void Amcl::UpdateFilter(HypPose &hyp_pose,
  ...
  if (resampled_ || force_publication_) {
    ...
    if (max_weight > 0.0) {
      ...
      ROS_INFO("==amcl==hyp_pose_==x:%f",hyp_pose.pose_mean[0]);
      ROS_INFO("==amcl==hyp_pose_==y:%f",hyp_pose.pose_mean[1]);
      hyp_pose.pose_mean = hyps[max_weight_hyp].pf_pose_mean;
      ROS_INFO("==amcl==hyps[max_weight_hyp].pf_pose_mean==x:%f",hyps[max_weight_hyp].pf_pose_mean[0]);
      ROS_INFO("==amcl==hyps[max_weight_hyp].pf_pose_mean==y:%f",hyps[max_weight_hyp].pf_pose_mean[1]);
      hyp_pose.pose_set_cov = set->covariant;
      ROS_INFO("==amcl==hyp_pose_==x1:%f",hyp_pose.pose_mean[0]);
      ROS_INFO("==amcl==hyp_pose_==y1:%f",hyp_pose.pose_mean[1]);
      publish_pose_ = true;
      update_tf_ = true;
    } else {
